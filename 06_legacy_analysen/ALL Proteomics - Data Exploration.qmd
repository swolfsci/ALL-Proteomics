...
uniprotid_to_hgnc_symbol[correlated_proteins] %in% -> swi_snf_vec
...
cor_map_adj <- magrittr::set_colnames(cor_map_adj, uniprotid_to_hgnc_symbol[colnames(cor_map_adj)])
cor_map_adj <- magrittr::set_rownames(cor_map_adj, uniprotid_to_hgnc_symbol[colnames(cor_map_adj)])

is.na(colnames(cor_map_adj))

cor_map_adj <- cor_map[!is.na(colnames(cor_map_adj)), !is.na(colnames(cor_map_adj))]


cor_map_adj[cor_map_adj < 0.5] <- 0

col_vec <- colnames(cor_map_adj) %in% c("SMARCA4", "SMARCC2", "SMARCD1", "SMARCA5", "SMARCC1", "SMARCB1", "SMARCD2", "SMARCD3", "SMARCD1", "SMARCA2", "SMARCA1", "SMARCAL1", "ARID1A", "ARID1B", "ACTL6A", "ACTL6B", "SMARCE1")

g <- igraph::graph_from_adjacency_matrix(cor_map_adj, mode = "undirected", weighted = T, diag = F)

igraph::plot.igraph(g, vertex.size = 5, edge.width = 1.3, vertex.color = col_vec, margin = 0, frame.width = 3)
...
igraph::plot.igraph(
  ...
  edge.width = 1,
  edge.color = rgb(0, 0, 0, 0.4),  # some transparency
  margin = 0,
...
st_judes_mut %>% 
  mutate(swi_snf = stringr::str_detect(Gene_VEP, "SMARC")) %>% 
  group_by(PatientID) %>% 
  arrange(desc(swi_snf), .by_group = T) %>% 
  slice_head(n = 1) %>% 
  ungroup() %>% 
  summarize(n(), .by=c("swi_snf"))
