...
We want to include some of the samples into the final analysis, precisely those with at least 70% viability/CD45dim/CD19high blasts (n = 13, denoted as "good pilot samples".

```{r}
meta.all %>% filter(viability >= 0.7 & cd45_blasts >= 0.7 & cd19_blasts >= 0.7 & alive_cells_total >= 1e6) %>% 
  dplyr::select(ID) %>% 
  writexl::write_xlsx("good_pilot_samples.xlsx")
```

# Amend 02/2025

calculate a correlation heatmap for the data set and focus on
