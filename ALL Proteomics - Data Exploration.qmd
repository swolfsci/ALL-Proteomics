...
ComplexHeatmap::HeatmapAnnotation(df = as.data.frame(meta.all) %>% dplyr::select(subtyp, thawing_date:pass)) -> all.heatmap_annotation
...
cog_rnaseq_topup %>% 
  left_join(cog_symbol_to_uniprot, by=c("geneName" = "SYMBOL")) %>% 
...
We can visualize the GSVA results in a simple heatmap and split the columns (=samples) based on the proteomic groupes that we derived from the community detection.
