...
prot.all.slc.irs.merge.filtered.10PCs %>% 
  ...
  rstatix::cor_test(vars = c(PC1:PC10), vars2=c(cells_per_ml:pass), method = "spearman") %>% 
  rstatix::adjust_pvalue() %>% 
  rstatix::add_significance() %>% 
...
This shows that the 1st PC is strongly correlated with viability and, to a lesser extent, the amount of CD19+ blasts. PC3 is moderately correlated with cellular content, protein concentration and the integrated QC metric "pass".
