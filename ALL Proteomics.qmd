...
```{r}
prot.all.slc[, grepl(pattern = "uniprotid", colnames(prot.all.slc.merge))] %>% 
  log2() %>% 
...
prot.all.slc.irs.merge.filtered %>% 
  ...
  as_tibble(rownames = "sample") %>%
  mutate(plex = stringr::str_extract(string = sample, pattern = "6\\d{2}")) %>% 
  left_join(prot.all.slc.irs.merge.missingness_by_plex, by="plex") %>% 
...
Two samples, 11103 and 26511 are not re-identified in the data set.
...
This leaves a final data set with a dimensionality of `dim(prot.all.slc.irs.merge.filtered)`
...
This shows that the 1st PC is strongly correlated with viability and, to a lesser extent, the amount of CD19+ blasts. PC3 is moderately correlated with cellular content and protein concentration as is PC4.
...
```{r}
meta.all.filtered %>% 
  rstatix::pairwise_wilcox_test(viability ~ thawing_date, ref.group = "2022-01-20")

meta.all.filtered %>% 
  rstatix::pairwise_wilcox_test(cells_per_ml ~ thawing_date, ref.group = "2022-01-20")

meta.all.filtered %>% 
  rstatix::pairwise_wilcox_test(alive_cells_per_ml ~ thawing_date, ref.group = "2022-01-20")

meta.all.filtered %>% 
  rstatix::pairwise_wilcox_test(cells_total ~ thawing_date, ref.group = "2022-01-20")

meta.all.filtered %>% 
  rstatix::pairwise_wilcox_test(alive_cells_total ~ thawing_date, ref.group = "2022-01-20")

meta.all.filtered %>% 
  rstatix::pairwise_wilcox_test(cd45_blasts ~ thawing_date, ref.group = "2022-01-20")

meta.all.filtered %>% 
  rstatix::pairwise_wilcox_test(cd19_blasts ~ thawing_date, ref.group = "2022-01-20")

```

However, none of the sample parameters is significantly different when comparing all thawing dates to the 2022-01-20.
